...
def book_author_search():
    for widgets in frameMain.winfo_children():
        widgets.destroy()
    
    book_author_input = entry1.get()
    book = ref.get()
    sorted_dict = {}

    for i in list(book.keys()):
        string_count = len(book_author_input)
        if str(book[i]['author'])[:string_count].lower() == book_author_input.lower():
...
Label(root, text = "Search By Author", font = ("Helvetica 10 underline"), fg = "black").pack(side = TOP, anchor = "n")

entry1 = Entry(root, width = 40)
entry1.insert(0, "Enter Author Name")
entry1.pack(pady = 25, side=TOP, anchor = "n")
button1 = Button(text = "Find Author", command = book_author_search).pack(side = TOP, anchor = "n")
